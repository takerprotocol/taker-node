...
use taker_common_node::cli_opt::{BackendType, BackendTypeConfig, RpcConfig};
...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
		Ok(match id {
			"dev" => Box::new(taker_dev_node::chain_spec::development_config()),
			"testnet_local" => Box::new(taker_testnet_node::chain_spec::testnet_config()),
			"testnet" => Box::new(taker_testnet_node::chain_spec::ChainSpec::from_json_file(
				std::path::PathBuf::from("./specs/taker-testnet.json"),
			)?),
			"mainnet_local" => Box::new(taker_mainnet_node::chain_spec::mainnet_config()),
			"mainnet" => Box::new(taker_mainnet_node::chain_spec::ChainSpec::from_json_file(
				std::path::PathBuf::from("./specs/taker-mainnet.json"),
			)?),
			path => Box::new(taker_dev_node::chain_spec::ChainSpec::from_json_file(
	...
	fn native_runtime_version(chain_spec: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		if chain_spec.is_dev() {
			&taker_dev_runtime::VERSION
		} else
		if chain_spec.is_testnet() {
			&taker_testnet_runtime::VERSION
		} else if chain_spec.is_mainnet() {
			&taker_mainnet_runtime::VERSION
		} else {
			&taker_dev_runtime::VERSION
		}
	}
			...
			if chain_spec.is_dev() {
				runner.async_run(|config| {
					let PartialComponents { client, task_manager, import_queue, .. } =
						taker_dev_node::service::new_partial(&config, &rpc_config)?;
					Ok((cmd.run(client, import_queue), task_manager))
				})
			} else if chain_spec.is_testnet() {
				runner.async_run(|config| {
					let PartialComponents { client, task_manager, import_queue, .. } =
						taker_testnet_node::service::new_partial(&config, &rpc_config)?;
					Ok((cmd.run(client, import_queue), task_manager))
				})
			} else if chain_spec.is_mainnet() {
				runner.async_run(|config| {
					let PartialComponents { client, task_manager, import_queue, .. } =
						taker_mainnet_node::service::new_partial(&config, &rpc_config)?;
					Ok((cmd.run(client, import_queue), task_manager))
				})
			} else {
				runner.async_run(|config| {
			...
			runner.sync_run(|config| {
				...
				match cmd {
					BenchmarkCmd::Pallet(cmd) => {
						...
						cmd.run::<taker_dev_runtime::Block, taker_dev_node::service::dev::ExecutorDispatch>(config)
					},
		...
		None => {
			...
			if chain_spec.is_dev() {
				runner.run_node_until_exit(|config| async move {
					taker_dev_node::service::new_full(config, rpc_config)
						.map_err(sc_cli::Error::Service)
			})} else if chain_spec.is_testnet() {
				runner.run_node_until_exit(|config| async move {
					taker_testnet_node::service::new_full(config, rpc_config)
						.map_err(sc_cli::Error::Service)
				})
			} else if chain_spec.is_mainnet() {
				runner.run_node_until_exit(|config| async move {
					taker_mainnet_node::service::new_full(config, rpc_config)
						.map_err(sc_cli::Error::Service)
				})
			} else {
				runner.run_node_until_exit(|config| async move {
