...
use taker_common_node::{
	...
	rpc::{FullDeps, GrandpaDeps, SpawnTasksParams, TracingConfig, BabeDeps},
...
pub mod dev {
	pub use taker_dev_runtime::RuntimeApi;
	...
	impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
		...
		type ExtendHostFunctions = fp_ext::bool_network_evm_ext::HostFunctions;

		fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
			taker_dev_runtime::api::dispatch(method, data)
		}

		fn native_version() -> sc_executor::NativeVersion {
			taker_dev_runtime::native_version()
		}
	}
}
...
type FullClient = sc_service::TFullClient<
	Block,
	dev::RuntimeApi,
	NativeElseWasmExecutor<dev::ExecutorDispatch>,
>;
...
> {
	...
	Ok(sc_service::PartialComponents {
		...
		select_chain,
		transaction_pool,
		other: (block_import, grandpa_link, frontier_backend, telemetry, babe_link, babe_worker_handle),
	})
}
...
pub fn new_full_base(
	...
	let sc_service::PartialComponents {
		...
		select_chain,
		transaction_pool,
		other: (babe_block_import, grandpa_link, frontier_backend, mut telemetry, babe_link, babe_worker_handle),
	...
	if is_authority {
		...
		let babe = sc_consensus_babe::start_babe(
			babe_config
		)?;
...
) -> impl Fn(DenyUnsafe, sc_rpc::SubscriptionTaskExecutor) -> Result<RpcModule<()>, sc_service::Error>
{
	...
	let overrides = taker_common_node::rpc::overrides_handle(client.clone());
	...
	let rpc_extensions_builder = move |deny_unsafe, subscription_executor| {
		let deps = FullDeps {
			...
			babe: BabeDeps {
				babe_worker_handle: builder.babe_worker_handle.clone(),
				keystore: builder.keystore.clone(),
			},
