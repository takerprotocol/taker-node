...
impl<T: Config> fungible::Unbalanced<T::AccountId> for Pallet<T> {
	...
	fn write_balance(
		...
		let (result, maybe_dust) = Self::mutate_account(who, |account| -> DispatchResult {
			...
			ensure!(reduction <= max_reduction, Error::<T>::InsufficientBalance);
...
impl<T: Config> fungible::Mutate<T::AccountId> for Pallet<T> {
	fn done_mint_into(who: &T::AccountId, amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Minted { who: who.clone(), amount });
	}
	fn done_burn_from(who: &T::AccountId, amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Burned { who: who.clone(), amount });
	}
	fn done_shelve(who: &T::AccountId, amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Suspended { who: who.clone(), amount });
	}
	fn done_restore(who: &T::AccountId, amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Restored { who: who.clone(), amount });
	}
...
impl<T: Config> fungible::UnbalancedHold<T::AccountId> for Pallet<T> {
...
impl<T: Config> fungible::Balanced<T::AccountId> for Pallet<T> {
	...
	fn done_deposit(who: &T::AccountId, amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Deposit { who: who.clone(), amount });
	}
	fn done_withdraw(who: &T::AccountId, amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Withdraw { who: who.clone(), amount });
	}
	fn done_issue(amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Issued { amount });
	}
	fn done_rescind(amount: Self::Balance) {
		Self::deposit_event(Event::<T>::Rescinded { amount });
	}
}

impl<T: Config> fungible::BalancedHold<T::AccountId> for Pallet<T> {}

// impl<T: Config> AccountTouch<(), T::AccountId> for Pallet<T> {
// 	type Balance = T::Balance;
