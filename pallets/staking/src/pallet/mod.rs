...
pub mod pallet {
	...
	pub const STORAGE_VERSION: StorageVersion = StorageVersion::new(13);
	...
	pub enum Event<T: Config> {
		...
		Bonded(T::AccountId, BalanceOf<T>, RewardDestination<T::AccountId>),
		...
		Validator(T::AccountId, ValidatorPrefs),
		/// A new force era mode was set.
		ForceEra { mode: Forcing },
		/// A controller has changed payee
		ChangePayee(T::AccountId, RewardDestination<T::AccountId>),
		...
		/// The era payout to account.
		EraRewardInfo(EraIndex, T::AccountId, BalanceOf<T>),
		/// The era payout to account [validator, account, commission, staking].
		EraRewardInfoEx(EraIndex, T::AccountId, T::AccountId, BalanceOf<T>, BalanceOf<T>),
	}
		...
		fn on_finalize(n: BlockNumberFor<T>) {
			...
			if let Some(mut active_era) = Self::active_era() {
				if active_era.start.is_none() {
					...
					active_era.start_block = Some(n.saturated_into());
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn bond(
			...
			Self::deposit_event(Event::<T>::Bonded(stash.clone(), value, payee));
		...
		pub fn bond_extra(
			...
			if let Some(extra) = stash_balance.checked_sub(&ledger.total) {
				...
				Self::deposit_event(Event::<T>::BondExtra(stash, extra));
			}
			Ok(())
		}
		...
		pub fn validate(origin: OriginFor<T>, prefs: ValidatorPrefs) -> DispatchResult {
			...
			Self::deposit_event(Event::<T>::Validator(ledger.stash, prefs));
			Ok(())
		}
		...
		pub fn nominate(
			...
			Self::deposit_event(Event::<T>::Nominator(controller, targets.to_vec()));
			Ok(())
		}
		...
		pub fn set_payee(
			...
			let stash = &ledger.stash;
			<Payee<T>>::insert(stash, payee.clone());
			Self::deposit_event(Event::<T>::ChangePayee(controller, payee));
			Ok(())
		}
		...
		pub fn payout_stakers(
			...
			Self::do_payout_stakers(sender, era)
		}
		...
		pub fn rebond(
			...
			Self::deposit_event(Event::<T>::ReBond {
